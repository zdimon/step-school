(function( $ ) {
    $.fn.quizPlugin = function() {
       ...
       this.find('#btn btn-primary d-flex align-items-center Sendp-2').on('click',this.sendMsg);

       app.sendMsg = function(){
        let data = {
            playername: sessionStorage.getItem('username'),
            message: 'Hello'
        };
        $.post('http://${this.serverUrl}/v1/quiz/sticker/list',data)
       }   
       return app;
